...
Use ``OllamaProvider`` when you need native local-runtime APIs such as `/api/tags`, `/api/show`, `/api/pull`, or `/api/version`. Keep ``OpenAIProvider`` with `.ollama()` for the simple OpenAI-compatible chat path.
